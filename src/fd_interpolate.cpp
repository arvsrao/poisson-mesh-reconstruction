  ...
  Eigen::SparseMatrix<double> & W)
{
    ...
    for (int idx = 0; idx < P.rows(); idx++) {
        ...
        W.insert(idx, i *j *k)                     = (1 - x_d) * (1 - y_d) * (1 - z_d); // a
        W.insert(idx, (i + 1) * j * k)             = x_d * (1 - y_d) * (1 - z_d);       // b
        W.insert(idx, (i + 1) * (j + 1) * k)       = x_d * y_d * (1 - z_d);             // c
        W.insert(idx, i *(j + 1) * k)              = (1 - x_d) * y_d * (1 - z_d);       // d
        W.insert(idx, i *j *(k + 1))               = (1 - x_d) * (1 - y_d) * z_d;       // e
        W.insert(idx, (i + 1) * j * (k + 1))       = x_d * (1 - y_d) * z_d;             // f
        W.insert(idx, (i + 1) * (j + 1) * (k + 1)) = x_d * y_d * z_d;                   // g
        W.insert(idx, i *(j + 1) * (k + 1))        = (1 - x_d) * y_d * z_d;             // t
    }
